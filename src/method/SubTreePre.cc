#include "../../include/Tree/SubTreePre.h"

SubTreePre::SubTreePre()
    : chunkCache(1024)
{
...
Chunk_t SubTreePre::CutGreedy(uint64_t BasechunkId, const Chunk_t Targetchunk, uint64_t HitSF)
{
...
uint8_t *SubTreePre::xd3_encode_buffer(const uint8_t *targetChunkbuffer, size_t targetChunkbuffer_size, const uint8_t *baseChunkBuffer, size_t baseChunkBuffer_size, size_t *deltaChunkBuffer_size, uint8_t *tmpbuffer)
{
...
void SubTreePre::StatsHit(uint64_t HitID)
{
...
void SubTreePre::SubTree(uint64_t HitSF, uint64_t HitFirstLayerID)
{
...
Chunk_t SubTreePre::xd3_recursive_restore_BL_time(uint64_t BasechunkId)
{
    cacheAccessCount++;
    auto [cachedDataPtr, found] = chunkCache.TryGet(BasechunkId);
    if (found && cachedDataPtr)
    {
        cacheHitCount++;
        Chunk_t cachedChunk;
        cachedChunk.chunkID = BasechunkId;
        cachedChunk.chunkSize = cachedDataPtr->size();
        cachedChunk.saveSize = cachedDataPtr->size();
        cachedChunk.chunkPtr = (uint8_t *)malloc(cachedDataPtr->size());
        cachedChunk.basechunkID = dataWrite_->chunklist[BasechunkId].basechunkID;
        cachedChunk.FirstChildID = dataWrite_->chunklist[BasechunkId].FirstChildID;
        cachedChunk.FirstBroID = dataWrite_->chunklist[BasechunkId].FirstBroID;
        cachedChunk.BeforeFit = dataWrite_->chunklist[BasechunkId].BeforeFit;
        cachedChunk.HitCount = dataWrite_->chunklist[BasechunkId].HitCount;
        cachedChunk.deltaFlag = dataWrite_->chunklist[BasechunkId].deltaFlag;
        cachedChunk.loadFromDisk = false;
        cachedChunk.HeaderFlag = dataWrite_->chunklist[BasechunkId].HeaderFlag;
        cachedChunk.NameExist = dataWrite_->chunklist[BasechunkId].NameExist;
        memcpy(cachedChunk.chunkPtr, cachedDataPtr->data(), cachedDataPtr->size());
        return cachedChunk;
    }
