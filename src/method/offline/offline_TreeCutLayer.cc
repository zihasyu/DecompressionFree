#include "../../../include/Offline/offline_treecutlayer.h"

OfflineTreeCutLayer::OfflineTreeCutLayer()
{
...
void OfflineTreeCutLayer::ProcessTrace()
{
    string tmpChunkHash;
    ...
    while (true)
    {
        ...
        if (recieveQueue->Pop(tmpChunk))
        {
            ...
            if (findRes == -1)
            {
                ...
                // OfflineTreeCutLayer get superfeature & get time
...
Chunk_t OfflineTreeCutLayer::CutGreedy(uint64_t BasechunkId, const Chunk_t Targetchunk, SuperFeatures sfs)
{
...
uint8_t *OfflineTreeCutLayer::xd3_encode_buffer(const uint8_t *targetChunkbuffer, size_t targetChunkbuffer_size, const uint8_t *baseChunkBuffer, size_t baseChunkBuffer_size, size_t *deltaChunkBuffer_size, uint8_t *tmpbuffer)
{
...
void OfflineTreeCutLayer::StatsHit(uint64_t FatherID, uint64_t HitID, SuperFeatures sfs)
{
