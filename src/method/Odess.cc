...
Odess::Odess() {
  // cout << " Chunk_t is " << sizeof(Chunk_t) << " Chunk_t_ori is " <<
  // sizeof(Chunk_t_odess) << " <super_feature_t, unordered_set<string>> is " <<
  // sizeof(super_feature_t);
  lz4ChunkBuffer = (uint8_t *)malloc(CONTAINER_MAX_SIZE * sizeof(uint8_t));
  mdCtx = EVP_MD_CTX_new();
  hashBuf = (uint8_t *)malloc(CHUNK_HASH_SIZE * sizeof(uint8_t));
  deltaMaxChunkBuffer =
      (uint8_t *)malloc(2 * CONTAINER_MAX_SIZE * sizeof(uint8_t));
  SFindex = new unordered_map<string, vector<int>>[FINESSE_SF_NUM];
}

Odess::Odess(int offlineMethod) : offlineMethod(offlineMethod) {
  lz4ChunkBuffer = (uint8_t *)malloc(CONTAINER_MAX_SIZE * sizeof(uint8_t));
  mdCtx = EVP_MD_CTX_new();
  hashBuf = (uint8_t *)malloc(CHUNK_HASH_SIZE * sizeof(uint8_t));
  deltaMaxChunkBuffer =
...
void Odess::ProcessTrace() {
    ...
    if (recieveQueue->Pop(tmpChunk)) {
      ...
      if (findRes == -1) {
        ...
        if (tmpChunk.chunkSize > 60) {
          ...
          superfeature =
              table.feature_generator_.GenerateSuperFeatures(tmpChunkContent);
        ...
        // unique chunk & delta chunk
        {
          ...
          uint8_t *deltachunk =
              xd3_encode(tmpChunk.chunkPtr, tmpChunk.chunkSize,
          ...
          } else if (tmpChunk.saveSize > tmpChunk.chunkSize) // error
          {
            ...
            tmpChunkLz4CompressSize = LZ4_compress_fast(
                (char *)tmpChunk.chunkPtr, (char *)lz4ChunkBuffer,
        ...
        // Basechunk id - [chunkid1, chunkid2, ...]
        if (offlineMethod >= 0) {
  ...
  if (ads_Version >= 100) {
    if (offlineMethod >= 0 && rootChunkMap != nullptr) {
      ...
      std::cout << "===== Leaf Node Count for Version " << ads_Version - 1
                << " =====" << std::endl;
      std::cout << "Total unique nodes in map: " << allNodes.size()
                << std::endl;
