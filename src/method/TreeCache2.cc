    ...
    : chunkCache(1024, 64)
{
    ...
    // 初始化双缓存系统
    currentVersionCache = new SequentialSlidingCache("./SeqCache_Current.dat");
    previousVersionCache = new SequentialSlidingCache("./SeqCache_Previous.dat");
}
...
TreeCache2::~TreeCache2()
{
    ...
    delete currentVersionCache;
    delete previousVersionCache;
}
    ...
    while (true)
    {
        ...
        if (recieveQueue->done_ && recieveQueue->IsEmpty())
        {
            cout << "Version " << ads_Version
                 << " LRU Cache Stats - Hits: " << cacheHitCount
                 << " Accesses: " << cacheAccessCount
                 << " Hit Rate: " << (float)cacheHitCount / cacheAccessCount * 100 << "%"
                 << endl;

            cout << "Sequential Cache Stats - Hits: " << seqCacheHitCount
                 << " Accesses: " << seqCacheAccessCount
                 << " Hit Rate: " << (float)seqCacheHitCount / seqCacheAccessCount * 100 << "%"
                 << endl;
...
Chunk_t TreeCache2::xd3_recursive_restore_BL_time(uint64_t BasechunkId)
{
    ...
    if (chunkCache.tryGet(BasechunkId, cachedData))
    {
        ...
        basechunk.chunkPtr = (uint8_t *)malloc(cachedData.size());
        basechunk.FirstChildID = dataWrite_->chunklist[BasechunkId].FirstChildID;
        memcpy(basechunk.chunkPtr, cachedData.data(), cachedData.size());
    ...
    // 这是一个根节点，为下个version缓存完整信息
    // std::vector<uint8_t> chunkData(basechunk.chunkPtr, basechunk.chunkPtr + basechunk.chunkSize);
    // currentVersionCache->insertRootChunk(BasechunkId, chunkData);
...
Chunk_t TreeCache2::CutGreedy(uint64_t BasechunkId, const Chunk_t Targetchunk, SuperFeatures sfs)
{
    ...
    std::vector<uint8_t> cachedData;
    seqCacheAccessCount++;
    if (tryGetFromSequentialCache(BasechunkId, cachedData))
    {
        seqCacheHitCount++;
        basechunk.chunkID = BasechunkId;
        basechunk.chunkSize = cachedData.size();
...
void TreeCache2::switchVersionCache()
{
    cout << "Switching version cache..." << endl;

    // 打印当前缓存统计
    currentVersionCache->printStats();

    // 交换缓存：当前收集的变成下个version要用的，上个version的清空
    SequentialSlidingCache *temp = previousVersionCache;
    previousVersionCache = currentVersionCache;
    currentVersionCache = temp;

    // 清空新的currentVersionCache为下个version做准备
    currentVersionCache->clearCache();

    cout << "Version cache switched successfully." << endl;
}
bool TreeCache2::tryGetFromSequentialCache(uint64_t chunkID, std::vector<uint8_t> &data)
{
    return previousVersionCache->tryGetSequential(chunkID, data);
}