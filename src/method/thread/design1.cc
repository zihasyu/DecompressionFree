#include "../../../include/Thread/design1.h"
...
void Design1::ProcessTrace()
{
    using namespace std;
    using namespace std::chrono;

    ThreadSafeQueue<RestoredChunk1> chunkQueue;
    ...
    std::thread restoreThread([&]() {
        for (size_t i = 0; i < totalChunks; i++)
        {
            ...
            chunkQueue.push(RestoredChunk1{i, tmpChunk, superfeature});
        }
...
Chunk_t Design1::CutGreedy(uint64_t BasechunkId, const Chunk_t Targetchunk)
{
...
uint8_t *Design1::xd3_encode_buffer(const uint8_t *targetChunkbuffer, size_t targetChunkbuffer_size, const uint8_t *baseChunkBuffer, size_t baseChunkBuffer_size, size_t *deltaChunkBuffer_size, uint8_t *tmpbuffer)
{
