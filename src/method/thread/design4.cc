#include "../../../include/Thread/design4.h"
#include <cstdint>

namespace
{
void ResetChunkForRebuild(Chunk_t &chunk)
{
    chunk.basechunkID = -1;
...
void Design4::SetHistoricalSource(dataWrite *historicalDataWrite, uint64_t historicalChunkBoundary)
{
    historicalDataWrite_ = historicalDataWrite;
...
Chunk_t Design4::LoadSourceChunk(uint64_t chunkId)
{
    ...
    {
        restoredChunk = source->xd3_recursive_restore_offline_time(chunkId);
    }
    else
    {
        restoredChunk = source->Get_Chunk_Info(chunkId);
    }
    ...
    ResetChunkForRebuild(resultChunk);
...
void Design4::ProcessTrace()
{
    ...
    if (rootChunkMap == nullptr)
    {
        cout << "rootChunkMap is nullptr in Design4::ProcessTrace" << endl;
        return;
    }
    ...
    std::thread restoreThread([&]()
        ...
        std::function<void(uint64_t)> process_tree = [&](uint64_t rootId) {
            ...
            while (!bfsQueue.empty())
            {
                ...
                chunkQueue.push(RestoredChunk4{rootId, cid, tmpChunk, item.initialRootId});
            }
...
uint8_t *Design4::xd3_encode_buffer(const uint8_t *targetChunkbuffer, size_t targetChunkbuffer_size, const uint8_t *baseChunkBuffer, size_t baseChunkBuffer_size, size_t *deltaChunkBuffer_size, uint8_t *tmpbuffer)
{
...
void Design4::StatsHit(uint64_t FatherID, uint64_t HitID, uint64_t BasechunkID)
{
...
Chunk_t Design4::CutGreedy(uint64_t BasechunkId, const Chunk_t Targetchunk)
{
